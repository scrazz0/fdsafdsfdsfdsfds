...
const Header = () => {
  ...
  useEffect(() => {
    // This is a mock login check. In a real app, you'd check a token.
        ...
        <div className="hidden md:flex items-center space-x-8">
          {NAV_LINKS.map(link => (
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) =>
                `font-medium transition-colors duration-200 hover:text-primary dark:hover:text-primary-dark ${isActive ? 'text-primary dark:text-primary-dark' : 'text-text-light dark:text-text-dark'}`
              }
            >
              {t(link.key)}
            </NavLink>
